(function initMeetingWorkspace(global) {
  ...
  async function refreshWorkspace(hydrateSelection) {
    ...
    try {
      ...
      state.sessionState = await namespace.storage.getMeetingState(state.params.sessionId);
      if (!state.params.sessionId) {
        state.params.sessionId = normalizeText(state.sessionState.session.sessionId);
      }
  ...
  async function startCapture() {
    if (!(Number(state.params.tabId) > 0) || !state.params.sessionId) {
      renderNotice("detailNotice", "이 페이지는 현재 대화에서 다시 열어야 녹음을 시작할 수 있어요.", "error");
      return;
    }
    try {
      renderNotice("detailNotice", "녹음을 시작하는 중입니다.", "highlight");
      await namespace.meetingBridge.startMeetingCapture({
        captureMode: "tab-audio",
        sessionId: state.params.sessionId,
        tabId: state.params.tabId,
        title: state.params.title || state.sessionState.session.title || namespace.session.formatSessionLabel(state.params.sessionId),
      });
  ...
  async function stopCapture() {
    if (!state.params.sessionId) {
      renderNotice("detailNotice", "현재 세션을 찾지 못했어요.", "error");
      return;
    }
    try {
      renderNotice("detailNotice", "녹음을 저장하는 중입니다.", "highlight");
      await namespace.meetingBridge.stopMeetingCapture({ sessionId: state.params.sessionId });
      await refreshWorkspace(false);
      ...
      const payload = await namespace.meetingBridge.createMeetingJob(
        namespace.meetingState.buildMeetingJobCreateInput(sessionState, {
          meeting: {
            ...
            title: state.params.title || sessionState.session.title || namespace.session.formatSessionLabel(state.params.sessionId),
          },
          options: {
            redaction: "none",
            speakerLabels: true,
  ...
  function handleStorageChange(changes, areaName) {
    if (areaName !== "local" || (!changes.meetingStateBySession && !changes.meetingState && !changes.cloudSync)) {
      return;
    }
  ...
  async function refreshMeetingRecords(meetingState) {
    ...
    if (!state.providerIdentity.available || !state.params.sessionId) {
    ...
    try {
      ...
      const nextMeetingState = namespace.meetingState.mergeMeetingState(normalizedMeetingState, {
        ...
        session: {
          ...
          sessionId: normalizeText(sessionPayload.sessionId) || normalizeText(state.params.sessionId),
      ...
      return await namespace.storage.setMeetingState(state.params.sessionId, nextMeetingState);
    } catch (error) {
  ...
  async function persistCurrentSessionSnapshot() {
    ...
    state.sessionState = await namespace.storage.setMeetingState(state.params.sessionId, nextMeetingState);
  }
  ...
  function render() {
    ...
    refs.pageTitle.textContent = state.params.title || sessionState.session.title || "아이노바 회의 작업실";
    refs.pageSummary.textContent = currentView.pageSummary;
    refs.sessionBadge.textContent = state.params.sessionId ? `세션 ${state.params.sessionId}` : "세션 없음";
    ...
    refs.recordList.innerHTML = records.length
      ...
      : `<div class="notice-box">아직 저장된 결과가 없습니다. 새 회의를 시작하면 이 대화의 결과 목록이 여기 쌓입니다.</div>`;
    ...
    return `
      <button type="button" class="record-item${active ? " is-active" : ""}" data-record-job-id="${escapeHtml(record.jobId)}">
        <div class="record-item__head">
          <strong class="record-item__title">${escapeHtml(record.title || record.sessionId || "회의 결과")}</strong>
  ...
  function buildCurrentView(sessionState) {
    ...
    if (captureStatus === "recording") {
      return createCurrentView("녹음 중", "recording", false, true, false, "현재 회의 녹음이 진행 중입니다.", "브라우저 탭 오디오를 녹음 중입니다. 종료 후 전사를 시작할 수 있습니다.", sessionState);
    }
    if (captureStatus === "captured" && jobStatus === "idle") {
      return createCurrentView("녹음 완료", "captured", false, false, true, "전사 시작 전 단계입니다.", "원본 녹음이 저장되었습니다. 전사 시작 버튼으로 다음 단계로 넘길 수 있습니다.", sessionState);
    }
    if (jobStatus === "queued") {
      return createCurrentView("대기", "queued", false, false, false, "전사 작업이 접수되었습니다.", "상세 진행 상태를 확인하는 중입니다.", sessionState);
    }
    if (jobStatus === "processing") {
      ...
      return createCurrentView("진행 중", "processing", false, false, false, "회의 전사를 처리 중입니다.", phase ? `${phase}${percent > 0 ? ` · ${percent}%` : ""}` : "상세 진행 상태를 확인하는 중입니다.", sessionState);
    }
    if (jobStatus === "succeeded") {
      const speakerCount = Math.max(0, Number(sessionState.transcript.speakerCount) || 0);
      return createCurrentView("완료", "succeeded", true, false, false, speakerCount > 0 ? `${speakerCount}명 화자 기준 결과가 준비되었습니다.` : "전사 결과가 준비되었습니다.", "왼쪽 리스트에서 다른 결과를 골라 다시 확인할 수 있습니다.", sessionState);
    }
    if (jobStatus === "failed" || captureStatus === "error") {
      return createCurrentView("오류", "failed", true, false, false, "회의 처리 중 문제가 생겼습니다.", normalizeText(sessionState.job.error || sessionState.capture.error) || "새 녹음으로 다시 시도할 수 있습니다.", sessionState);
    }
    return createCurrentView("대기", "idle", Boolean(Number(state.params.tabId) > 0 && state.params.sessionId), false, false, "새 회의를 시작하거나 기존 결과를 다시 확인할 수 있습니다.", Number(state.params.tabId) > 0 ? "현재 대화 기준으로 새 녹음을 시작할 수 있습니다." : "이 페이지를 현재 대화에서 다시 열면 바로 새 회의를 시작할 수 있습니다.", sessionState);
  }

  function createCurrentView(badgeLabel, badgeStatus, canStart, showStop, canTranscribe, summary, hint, sessionState) {
    return {
      badgeLabel,
      badgeStatus,
      canStart,
      ...
      hint,
      pageSummary: "패널은 결과 목록만 보여주고, 실제 회의 작업은 이 전용 화면에서 처리합니다.",
      showStop,
      sizeText: Number(sessionState.capture.sizeBytes) > 0 ? formatBytes(sessionState.capture.sizeBytes) : "-",
      summary,
      title: state.params.title || sessionState.session.title || "아이노바 회의 작업실",
  ...
  function parseParams(urlValue) {
    ...
    return {
      ...
      sessionId: normalizeText(url.searchParams.get("sessionId")),
      tabId: Math.max(0, Number(url.searchParams.get("tabId")) || 0),
