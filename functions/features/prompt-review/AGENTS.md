...
- `prompt-telling-v2` opt-in 요청에서는 6개 id 순서와 `core/refinement` group 메타, 서버 계산 총점이 고정적으로 내려오는지 확인한다.
...
- `legacy-v1`는 현재 4개 id `context/goal/constraints/output` 계약과 모델 총점을 유지하고, `prompt-telling-v2`는 `persona/reference/objective/mode/pointOfView/tone` id와 서버 계산 총점을 사용한다. 사용자에게 보이는 summary/check feedback/refinedPrompt에는 영문 기준명, 내부 약어, 대괄호 빈칸 표시를 내보내지 않는다.
