...
- `사용자 및 권한`은 기존 회원 후보를 서버에서 읽고 `ops_admin_users/{providerUserKey}`의 `status: active|inactive`만 변경한다. 이메일 직접 입력으로 새 관리자를 만드는 흐름은 기본 경로가 아니다.
- 회원 후보 목록은 `integration_inova_accounts_v2`, `integration_inova_accounts`, `integration_inova_feature_usage_user_months`, 기존 `ops_admin_users`의 identity snapshot을 합쳐 만든다. 화면은 이 후보 안에서 선택한 회원의 관리자 권한만 저장한다.
