(function initHostedMeetingWorkspace(global) {
  ...
  function bindEvents() {
    ...
    refs.retryMeetingNotesButton?.addEventListener("click", runWritableAction("회의 정리 다시 만들기", () => void controllers.mutations.retryMeetingNotes()));
    refs.copySegmentsButton?.addEventListener("click", () => void controllers.debug.copySegmentsText());
    refs.toggleLowQualitySegmentsButton?.addEventListener("click", () => {
      state.showLowQualitySegments = !state.showLowQualitySegments;
      applyRender();
    });
