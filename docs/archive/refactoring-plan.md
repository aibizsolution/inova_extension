...
> Archived reference. 이 문서는 기본 작업 시작 시 읽지 않으며, 최신 운영 기준으로 유지보수하지 않는다. 과거 version/release 판단 배경이 필요할 때만 참조한다.
...
- active `shared/storage.js`의 extension reload 감지는 에러 문자열만 기준으로 삼지 않는다. `chrome.runtime.id` 누락 또는 `chrome.runtime.lastError` 같은 runtime 신호를 먼저 보고, 메시지 문자열은 fallback으로만 유지한다.
...
- 현재 `1.x+` extension bundle은 manifest browser privilege surface도 고정한다. live `permissions`는 `storage`, `tabs`, `cookies`, `declarativeNetRequest`만 유지하며, `declarativeNetRequest`는 로컬 hosted panel direct iframe을 위해 i-Nova main-frame CSP에 local `frame-src`를 추가하는 용도로만 쓴다. icon-click grant UX가 없는 `activeTab`은 active surface에 두지 않는다. `host_permissions`, `extension_pages frame-src`, meeting workspace bridge match, frame proxy web-accessible match도 현재 계약 표면만 유지하고, 새 권한/origin/match는 browser-only capability 필요성이 확인될 때만 contract + manifest를 같이 늘린다.
...
- 현재 `1.x+` extension bundle은 release legacy runtime/helper도 활성 manifest에 싣지 않는다. `backup/legacy-panel/release-manager.js`, `backup/legacy-panel/shared/release-info.js`는 `0.4.4` 영향 판단용 reference로만 남고, active v2 bundle은 release count/view state를 hosted release controller 안에만 유지한다. active `shared/constants.js`와 `shared/firebase-config.js`는 release check interval이나 latest/history endpoint helper를 들지 않고, hosted release controller가 lane-local `../releases/latest.json` / `../releases/history.json` 경로와 자체 interval fallback을 소유한다.
...
- `package.json`에 browser E2E 보조 점검이나 fixture 검증 script를 추가하는 변경은 그 자체로 version lane 전환이나 release decision 변경이 아니다. 다만 기본 `npm.cmd run verify`에 포함되는 새 script는 active v2 lane 기준을 더 강하게 고정하는 목적이어야 하고, legacy backup 검증을 기본 verify로 다시 섞지 않는다.
...
- meeting transcription quality guard도 active `npm.cmd run verify` baseline에 포함한다. `scripts/verify-meeting-transcription-quality.js`는 같은 문장을 비정상적으로 반복하는 전사 결과를 한 번 재시도하고, 재시도 후에도 반복이면 성공 저장하지 않는 경계를 고정한다.
- meeting audio source policy guard도 active `npm.cmd run verify` baseline에 포함한다. `scripts/verify-meeting-audio-source-policy.js`는 OpenAI 25MB 업로드 제한보다 낮은 24MB source part target과 `gpt-4o-transcribe` 1400초 단일 오디오 제한보다 낮은 23분 chunk 전환 안전선을 고정한다.
- 같은 guard는 hosted chunk 기본값도 고정한다. chunked source는 12kHz mono WAV, 14분 target, 1.5초 overlap이며, 26분대 회의가 2개 part로 나뉘고 각 chunk WAV가 24MB target 아래에 머물러야 한다. 실제 경계는 target 주변 45초 안에서 low-energy 구간을 찾아 조정한다.
...
- local rehearsal에서 page DOM에 삽입되는 hosted panel iframe src는 direct loopback URL(`http://127.0.0.1:5000/*` 또는 `http://localhost:5000/*`)을 기본값으로 쓴다. i-Nova page CSP의 local `frame-src` 허용은 background `declarativeNetRequest` rule이 맡고, extension `content/frame-proxy.html?target=...` wrapper는 Playwright Bridge attach를 깨므로 emergency-only fallback으로만 유지한다.
