...
| 관련 데이터 경계 | `ops_admin_users`, `ops_admin_launches`, `ops_admin_sessions`; `사용자 및 권한` 회원 후보 read는 `integration_inova_accounts_v2`, `integration_inova_accounts`, `integration_inova_feature_usage_user_months` identity snapshot을 합쳐 쓴다. |
