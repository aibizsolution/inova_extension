...
- remote capability alias와 hosted page capability dispatch는 active `npm.cmd run verify` baseline에 포함한다. `scripts/verify-runtime-capability-router.js`는 background manifest alias 해석을, `scripts/verify-extension-capability-client.js`는 hosted capability client의 page/alias dispatch allowlist를 고정한다.
