...
- 다음 구조 목표는 새 backend action 추가가 extension 재배포로 이어지지 않게 하는 것이다. `background/panel-runtime-capability-router.js`와 `background/functions-runtime-config.js`는 bundled manifest 모델을 현재 extension-internal baseline으로 두고, 후속 작업은 `docs/remote-capability-manifest-plan.md` 순서대로 remote manifest fetch/cache -> endpoint/runtime config 원격화로 진행한다. 이 작업은 보안 경계 변경이므로 hosted가 raw URL을 background에 넘기는 방식으로 해결하지 않는다.
