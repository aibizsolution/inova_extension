...
- Storage Rules 변경은 hosted 정적 자산이나 functions runtime 변경과 별도 배포 표면이다. 운영 반영 보고에서는 `deploy:storage` 또는 storage가 포함된 `deploy:all`/`release:deploy:all` 필요 여부를 functions/hosting 배포와 구분해 적는다.
