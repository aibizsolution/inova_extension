...
- 현재 `1.x+` extension bundle은 dormant prompt import/export helper도 활성 manifest에 싣지 않는다. `shared/prompt-library.js`는 import/export 정규화 reference용으로만 남고, active content preload baseline은 `shared/provider-identity-cache.js`, generic `shared/storage.js`, `content/provider-identity-sensor.js`, hosted prompt controller 쪽에 둔다. dormant prompt CRUD/sync operation helper는 `backup/legacy-panel/shared/prompt-cloud-sync.js`, `backup/legacy-panel/shared/prompt-storage.js` backup reference로만 남긴다.
