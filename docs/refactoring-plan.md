...
- meeting audio source policy guard도 active `npm.cmd run verify` baseline에 포함한다. `scripts/verify-meeting-audio-source-policy.js`는 OpenAI 25MB 업로드 제한보다 낮은 24MB source part target과 duration-only single 전사 유지를 고정한다.
