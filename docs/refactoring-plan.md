...
- 현재 `1.x+` extension bundle은 prompt legacy runtime도 활성 manifest에 싣지 않는다. `content/features/prompt-library/files.js`, `content/features/prompt-library/cloud-sync-manager.js`, `content/features/prompt-library/prompt-manager.js`, `content/features/prompt-store/prompt-realtime-manager.js`, `content/features/prompt-store/store-manager.js`, `content/prompt-hub-state.js`, `content/prompt-hub-panel.js`, `content/prompt-hub-controller.js`, `content/prompt-hub-runtime.js`, `content/panel-prompt-controller.js`는 legacy reference/source로 남고, active v2 bundle은 `content/panel-v2-prompt-controller.js`로 review handoff와 minimal prompt snapshot만 유지한다.
