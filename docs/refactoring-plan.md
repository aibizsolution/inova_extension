...
- 현재 `1.x+` extension bundle은 dormant prompt import/export helper도 활성 manifest에 싣지 않는다. `shared/prompt-library.js`는 import/export 정규화와 storage/helper reference용으로만 남고, active content preload baseline은 `shared/cloud-sync.js`, `shared/provider-identity.js`, hosted prompt controller 쪽에 둔다.
