...
- 현재 `1.x+` extension bundle은 standalone activity/surface/lifecycle/bootstrap/render helper도 활성 manifest에 싣지 않는다. active open/visibility/surface/bootstrap/render bridge는 `content/panel-v2-shell-bridge.js` 한 파일로만 유지하고, `content/panel-v2-composition-controller.js`는 그 bridge wiring만 맡는다.
