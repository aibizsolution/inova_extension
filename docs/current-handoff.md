...
- active background root inventory도 이제 `browser-capability / cloud-api-client / inova-auth-client / meeting-workspace-capability / panel-auth-cache / panel-runtime-capability-router / panel-runtime-invoke / panel-session-capability / service-worker` 아홉 파일로 고정했고, `contracts/extension-contract.json` + `scripts/verify-contracts.js`가 그 경계를 같이 잠근다
