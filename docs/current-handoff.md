...
- Hosted close/Escape and external handle click no longer need the old hosted/content toggle calculation cycle. Hosted review consumes Escape first, and external handle clicks are sent only as panel events; hosted updates local panel-open state, persists the user preference through `uiPreferences.panelOpen`, and sends parent DOM visibility through `panel-chrome-sync`. Content no longer reads or writes panel-open `sessionStorage`; it sends the raw `uiPreferences.panelOpen` value only as the first hydration seed and then applies hosted chrome sync.
...
- It should not keep tool summary view models or panel open preference as independent extension state. The canonical active tool is no longer mirrored as `state.activeTool` or sent as a prebuilt top snapshot field. Global `state.open`, `state.preferredOpen`, and content `sessionStorage` panel-open persistence are gone; content mirrors hosted chrome-sync `open` only in `uiPreferences.panelOpen` and sends that raw preference as the initial hydration seed. Handle count and hosted close/Escape/external-handle open state are now computed by hosted and synced back through `panel-chrome-sync`; hosted no longer accepts later content snapshot `open` values after initial hydration.
...
- Hosted panel open persistence now writes `uiPreferences.panelOpen` from hosted. Content no longer owns `PANEL_OPEN_KEY`, `sessionStorage`, `preferredOpen`, or `persistOpen`; route/surface changes only affect `visible`, while open restoration follows the hosted preference.
...
- Step 4 has started with `shared/firestore-collections.js`; `shared/product-lane.js` and `shared/firebase-config.js` now consume the same browser-agnostic collection catalog instead of repeating prompt collection names. `shared/firestore-collections.js` and `shared/firebase-config.js` intentionally keep tiny early-load `trimString` / base-url helpers because popup loads them before `shared/session.js`; do not replace those with `namespace.session.normalizeText` unless the browser load order and corresponding verification are changed first.
...
- The next architecture slice for "no extension redeploy for backend actions" is documented in `docs/remote-capability-manifest-plan.md`. The current extension-internal baseline is the bundled manifest model plus trusted Hosting `capability-manifest.json` fetch/cache in `functions-runtime-config.js`; Phase 3 is remote endpoint/target resolution.
...
- active hosted runtime broker no longer exposes generic storage read/write mutation to the v2 panel; the live runtime contract now converges on `storage.read-panel-state`, `storage.write-ui-preferences`, `auth.issue-panel-session`, `capabilities.handshake`, `capabilities.invoke`, `functions.invoke-endpoint`, `browser.open-url`, and `meeting.*` capability actions only
