...
8. 상용 AI provider 우선순위는 OpenRouter first다. 회의 전사는 OpenRouter 실패 시 같은 JSON secret 안의 Gemini 설정을 Files API fallback으로 먼저 쓰고, OpenAI는 필요한 경우 마지막 fallback으로만 둔다. Secret Manager에는 provider별 개별 키를 만들지 않고 저장소 전용 JSON secret `INOVA_EXTENSION_AI_PROVIDER_CONFIG` 하나만 관리한다. 공용 `OPENAI_API_KEY` secret이나 `.env`에 i-Nova/OpenRouter/Gemini 키를 넣어 배포하지 않는다.
