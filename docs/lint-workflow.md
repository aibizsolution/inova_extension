...
- 현재 baseline에서는 `no-unused-vars`, `preserve-caught-error`를 활성화했고, `empty catch`는 허용한다. `no-control-regex`는 기존 코드 정리 전까지 보류한다.
...
- `eqeqeq` 같은 비교 일관성 규칙을 다음 hygiene 후보로 검토
