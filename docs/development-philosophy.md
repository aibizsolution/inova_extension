...
- fallback이나 cached/stale data를 보여주면 무엇이 실패했고 최신이 아닐 수 있는 범위를 사용자와 로그에 degraded 상태로 드러낸다.
- 저장, 전송, 생성, 분석, 동기화처럼 성공 여부가 중요한 작업은 대체값이나 재시도 실패를 성공 완료로 처리하지 않는다.
- 배포 누락, 계약·스키마 불일치, 권한 문제처럼 근본 원인이 있는 실패는 원인을 먼저 고치고, 즉시 고칠 수 없으면 명시적 error로 남긴다.
