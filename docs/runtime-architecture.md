...
- 역할: launch token 교환, hosted session 복원, 회의 녹음 시작/종료, 전사 접수, 결과 리스트, 선택한 결과의 발화 구간 상세 렌더링, 화자별 AI 정리, 전체 복사
- 특징: 회의 제어와 상세 보기를 확장 UI에서 분리한 메인 작업실이다. `meetingSessionToken`으로 회의 API를 직접 호출하고, 브라우저 표준 `getUserMedia + MediaRecorder` 경로로 마이크를 녹음한다.
...
5. hosted 회의 작업실은 URL hash 또는 storage에 있는 `meetingSessionToken`으로 부팅하고, hash 없는 clean URL만 직접 열면 hosted 세션이 없으므로 접근을 막고 패널에서 다시 열도록 안내한다.
...
1. hosted 회의 작업실이 방금 녹음한 blob을 inline payload로 바꾼다.
2. hosted 회의 작업실이 `meetingSessionToken`으로 `createInovaMeetingJob`을 직접 호출한다.
3. Functions는 임시 source object 업로드, OpenAI diarization, transcript artifact 저장, source cleanup을 수행한다.
4. hosted 회의 작업실은 `listInovaMeetingResults`, `getInovaMeetingJob`, `getInovaMeetingArtifact`를 다시 호출해 결과 리스트와 상세 transcript를 갱신한다.
