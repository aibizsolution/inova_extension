...
- 특징: active background root inventory도 `browser-capability.js`, `cloud-api-client.js`, `inova-auth-client.js`, `meeting-workspace-capability.js`, `panel-auth-cache.js`, `panel-runtime-capability-router.js`, `panel-runtime-invoke.js`, `panel-session-capability.js`, `service-worker.js`로 고정하고, 이 목록은 `contracts/extension-contract.json` + `scripts/verify-contracts.js`로 같이 잠근다.
