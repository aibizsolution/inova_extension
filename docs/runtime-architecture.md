...
- 특징: 장기 운영 목표는 backend action 추가 때문에 extension ZIP을 재배포하지 않는 것이다. 현재 `background/panel-runtime-capability-router.js`와 `background/functions-runtime-config.js`는 안전한 고정 baseline으로 유지하되, 다음 구조 개선은 `docs/remote-capability-manifest-plan.md`의 remote capability manifest 방향을 따른다. 단, 새 browser permission, 새 host permission, 새 page DOM adapter, 새 web-accessible asset처럼 extension 권한 자체가 바뀌는 경우는 계속 extension 재배포 대상이다.
