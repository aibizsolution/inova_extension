...
- 특징: 현재 대화 DOM을 읽고, 로컬 상태를 붙이고, 필요한 클라우드 호출은 background에 메시지로 위임한다. `content/main.js`는 composition root, `content/panel-v2-composition-controller.js`는 active state/composition/runtime/provider-identity bridge와 hosted-owned controller graph 조립을 맡고, `content/panel-v2-shell-bridge.js`는 active tool/activity/surface/lifecycle/bootstrap/render bridge를 맡고, hosted panel에 넘길 `panelSnapshot`과 `panelTrace`도 여기서 먼저 조립한다. `content/panel.js`는 host element lifecycle과 helper wiring만 맡고, iframe target/status/handshake/render batching은 `content/panel-host-runtime.js`, hosted bridge endpoint와 page event emit은 `content/panel-host-bridge.js`, host markup과 handle drag/click은 `content/panel-host-view.js`가 맡는다. panel iframe target 선택도 `shared/firebase-config.js`의 generic `firebaseConfig.panel.resolveRuntime()` helper를 통해 받아 feature-aware runtime 분기를 직접 들지 않는다. render bridge는 hosted tool state를 `panelSnapshot`으로 먼저 조립해 넘기고, top snapshot trace detail도 prebuilt `panelTrace`로 함께 넘기므로 `content/panel.js`는 더 이상 prompt review detail을 직접 읽지 않는다. top console trace allowlist와 summary formatting은 `content/panel-console-trace.js`가 맡고, `content/panel.js`는 `panel-console-trace`, `panel-host-runtime`, `panel-host-bridge`, `panel-host-view` helper를 주입받아 host 쪽 부수효과만 남긴다. `content/hosted-panel-bridge.js`는 hosted iframe postMessage contract와 active hosted request helper를 함께 맡는다. 현재 `1.0.0+` extension bundle은 panel을 `content/panel-v2-composition-controller.js`로 직접 부팅하고, `backup/legacy-panel/panel-composition-controller.js`, `backup/legacy-panel/panel-action-controller.js`, `backup/legacy-panel/panel-meeting-controller.js`, `backup/legacy-panel/meeting-manager.js`는 활성 manifest에 싣지 않는다. `1.0.0+` v2 lane은 shell/runtime/route/page adapter만 extension에 남기고, tool rail/header chrome 같은 hosted shell 결정은 더 이상 extension에서 계산하지 않는다. v2 bootstrap은 실제로 hosted panel이 아직 보내는 shell callback(`select-tool`, `search`, `bookmark`, `escape`, `tool-summary-sync`)만 host에 넘기고, release/meeting/prompt legacy action callback은 기본 surface에서 제외한다. active `content/hosted-panel-bridge.js`도 이에 맞춰 compact `tool summary sync`, `conversation bookmark`, shell/runtime/page contract만 유지하고, legacy `release-action`, `meeting-action`, prompt action request path는 더 이상 active `1.0.0` bundle에 남겨 두지 않는다. active page adapter는 feature별 임시 action 대신 `conversation.read-state`, `conversation.jump-item`, `composer.read-state`, `composer.apply-text`, `clipboard.write-text`, `debug.read-state`, `debug.set-enabled`, `debug.copy-log`, `debug.clear-log`, `trace.log` 같은 capability-shaped contract를 기준으로 수렴시키고, old action name은 호환 alias로만 유지한다. `DB/Functions` 계약을 바꾸지 않는 순수 panel migration이라면 판단 기준은 현재 v2 bundle이 정상 동작하는지다. v2 composition은 prompt bridge proxy를 다시 한 번 만들지 않고 active v2 prompt controller를 render/bootstrap wiring에만 직접 넘기되, live shell 계약 이름도 legacy `panelPromptController` 대신 `promptShellController`로 유지한다. legacy lane도 별도 prompt bridge proxy 없이 `backup/legacy-panel/panel-prompt-controller.js`를 shell/render/bootstrap에 직접 넘긴다. conversation도 v2에서는 bookmark item list와 empty/meta 문구를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/conversation-controller.js`가 page snapshot으로 직접 계산하며 extension은 count, activeId, refresh용 fingerprint만 남긴다. prompt도 v2에서는 prompt/store item list와 prompt activeTab/search state를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/prompt-library-controller.js`, `prompt-library-firestore-client.js`, `prompt-store-controller.js`, `prompt-review-controller.js`가 각 탭 상태와 prompt tab 전환/persistence, prompt/store/review action routing, import/reorder draft mutation을 직접 계산한다. active manifest는 `shared/prompt-library.js`도 더 이상 preload하지 않고, prompt import/export 정규화 helper는 dormant shared reference로만 남긴다. active `shared/cloud-sync.js`와 `shared/storage.js`도 provider-identity/local-state merge 같은 generic helper만 유지하고, prompt CRUD/sync operation builder는 `backup/legacy-panel/shared/prompt-cloud-sync.js`, `backup/legacy-panel/shared/prompt-storage.js` backup reference로만 남긴다. active prompt shell view는 `prompt-tool-view.js`가 맡고, interaction helper도 `prompt-tool-panel.js`가 맡는다. active hosted prompt tool shell은 review/store placeholder fallback 문구를 더 이상 끌고 가지 않고, `prompt-review-controller.js`와 `prompt-store-controller.js` state를 직접 주입받아 해당 탭을 렌더한다. v2 hosted panel은 더 이상 `prompt-hub-view.js` fallback을 로드하거나 렌더 fallback으로 쓰지 않고, active helper naming도 legacy `prompt-hub-*`를 끌고 가지 않는다. active hosted v2 panel은 이제 단일 `index.css`만 로드하고, dead `legacy-panel.css`와 `legacy-tools.css`에 있던 살아 있는 bookmark/tool-shell 스타일도 현재 hosted shell stylesheet 안으로 흡수했다. extension은 `content/panel-v2-prompt-controller.js`로 composer review float와 monotonic external review `requestId` handoff signal만 유지하고, review result/error/open/pending 상태는 다시 들고 있지 않는다. hosted review tab의 escape dismiss도 `prompt-review-controller.js`가 먼저 소비하고, generic `prompts/store` tool 선택은 `content/panel-v2-shell-bridge.js`가 `activeTool: "prompts"` + `activePromptTab` persistence로만 정규화한다. persisted legacy `activeTool: "store"` 값은 `shared/storage.js`에서 즉시 `activeTool: "prompts"` + `activePromptTab: "store"`로 흡수하고, active route/shell은 더 이상 store를 별도 tool id로 들고 있지 않는다. active v2 shell은 더 이상 hosted-owned prompt store load, storage listener, realtime/cloud sync sidecar를 extension에서 다시 깨우지 않고, hosted-owned release refresh/download lifecycle도 extension에서 다시 깨우지 않는다. active `createState()`와 `route-state-controller`도 hosted-owned prompt library/store/editor mirror를 다시 들지 않는다. active compact hosted residue는 feature별 top-level bucket 대신 `toolSummaries` 아래에 모으고, 현재는 count-only `toolSummaries.release`와 count-only `toolSummaries.meeting`만 유지한다. v2 composition은 이 compact `meeting` / `release` residue도 feature별 snapshot helper 대신 shared `toolSummaries` normalizer/helper로 정규화하고, v2 shell render도 meeting/release 전용 deps 대신 shared tool-summary callback으로만 그 residue를 읽는다. active `1.0.0+` manifest는 더 이상 `backup/legacy-panel/features/prompt-library/files.js`, `backup/legacy-panel/features/prompt-library/cloud-sync-manager.js`, `backup/legacy-panel/features/prompt-library/prompt-manager.js`, `backup/legacy-panel/features/prompt-store/prompt-realtime-manager.js`, `backup/legacy-panel/features/prompt-store/store-manager.js`, `backup/legacy-panel/prompt-hub-state.js`, `backup/legacy-panel/prompt-hub-panel.js`, `backup/legacy-panel/prompt-hub-controller.js`, `backup/legacy-panel/prompt-hub-runtime.js`, `backup/legacy-panel/panel-prompt-controller.js`를 활성 bundle에 싣지 않는다. default `verify`는 `scripts/verify-legacy-isolation.js`로 이 active/backup 경계도 같이 고정한다. 프롬프트 `내 요청` read path는 hosted panel 안에서 `auth.issue-prompt-panel`로 Firebase custom token을 발급받고, `integration_inova_accounts_v2.promptLibraryMeta` `onSnapshot` 뒤 `prompt_library_orders_v2`/`prompt_library_chunks_v2` direct read로 조립한다. 단순 목록 read를 위한 `loadInovaPromptLibraryV2` Functions fetch는 v2 hosted path에서 다시 쓰지 않는다. release도 v2에서는 latest/history/error 같은 상세 view-state를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/release-controller.js`가 직접 계산하며 extension은 rail count용 count-only compact summary만 남긴다. refresh/download 액션도 이제 hosted release controller가 직접 끝내고, extension은 runtime invoke의 `browser.open-url` broker만 맡는다. meeting도 v2에서는 회의 허브 item list와 action UI 상태를 extension snapshot이나 active extension state bucket에 싣지 않고, hosted `extension-v2/panel/meeting-hub-controller.js`가 `storage.get-state`, `auth.issue-meeting-panel`, hosted `meeting-firestore-client.js`, `meeting.open-*`, `meeting.*share*` runtime invoke로 목록/오류/degraded/pending/feedback/share 상태를 직접 계산한다. 목록 읽기는 hosted panel 안에서 Firebase custom token sign-in 뒤 Firestore `integration_inova_meetings` `onSnapshot`으로 이어지고, hosted meeting hub는 compact `meeting summary`를 generic top-panel `tool-summary-sync`로 다시 올려 extension의 count-only `toolSummaries.meeting`만 유지한다. 이 summary sync는 이제 rail count를 위한 outbound 신호로만 남고, hosted meeting hub는 echo된 `panelState.meetingTool` count를 lifecycle/state 정본으로 다시 쓰지 않는다. hosted meeting count는 bootstrap 중에도 `0`으로 시작하고, Firestore snapshot과 현재 hub memory로만 올라간다. active v2 createState는 legacy `meetingHub`/`meetingUi` bucket을 더 이상 들지 않고, 이 compact summary는 count-only state로 바로 normalize하며 `backup/legacy-panel/meeting-manager.js` merge helper를 다시 거치지 않는다. v2 hosted panel은 `meeting-action` top-panel fallback 없이 open/share/revoke를 자체적으로 끝내며, extension은 runtime broker와 브라우저 탭 열기만 맡는다. v2 shell도 route/storage/prompt-tab/surface/bootstrap 같은 sidecar/lifecycle 이벤트로는 더 이상 `backup/legacy-panel/meeting-manager.js` sync를 깨우지 않고, hosted `meeting-hub-controller`가 top-panel fingerprint echo 없이 meeting tool 재진입, hosted 문서 `visibility-visible`, explicit action 결과를 기준으로 load/refresh를 직접 판단한다. meeting storage listener는 v2 bootstrap에서 등록하지 않고, tool state/controller ownership은 hosted `extension-v2/panel/*`로 점진 이동한다. 회의 기능은 브라우저 쪽에서 `meetingStateByMeetingId` 상태만 유지하고, `backup/legacy-panel/meeting-manager.js`는 legacy reference/source로만 남는다. 회의 허브 목록은 별도 `chrome.storage.local` 캐시 대신 hosted Firestore subscription과 현재 메모리 상태를 우선한다. 프롬프트/스토어 쪽은 legacy lane에서 `backup/legacy-panel/panel-prompt-controller.js`가 `backup/legacy-panel/prompt-hub-*` shell과 `backup/legacy-panel/shared/prompt-cloud-sync.js`, `backup/legacy-panel/shared/prompt-storage.js`, `backup/legacy-panel/features/prompt-library/cloud-sync-manager.js`, `backup/legacy-panel/features/prompt-store/prompt-realtime-manager.js` wiring을 감싸고 panel shell도 직접 상대한다. `1.0.0+` v2 lane에서는 hosted prompt panel이 `prompt-tab-select`, `prompt-action`, `prompt-draft-change`, `store-action`, `import-file`, `move-prompt` 같은 top-panel fallback 없이 자체 tab selection과 action flow를 끝낸다. `1.0.0+` v2 lane은 `v2.*` local storage key와 `integration_inova_accounts_v2` prompt-library meta 문서를 우선한다. panel/read-state 계약은 공통으로 `source`, `degraded`, `degradedReason`, `dataFreshness(fresh|stale|empty)`를 함께 들고, stale cache를 success-like fallback으로 숨기지 않는다. 패널 디버그는 회의 전용이 아니라 현재 브라우저 탭 세션 기준 전역 버퍼를 쓰고, 사용자 관측은 top 콘솔 trace를 기준으로 한다.
