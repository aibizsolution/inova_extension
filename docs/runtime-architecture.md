...
- 특징: 현재 대화 DOM을 읽고, 로컬 상태를 붙이고, 필요한 클라우드 호출은 background에 메시지로 위임한다. `content/main.js`는 composition root, `content/panel-v2-composition-controller.js`는 active state/composition/runtime/provider-identity bridge와 hosted-owned controller graph 조립을 맡고, `content/panel-v2-shell-bridge.js`는 active activity/surface/lifecycle/bootstrap bridge를 맡고, `content/panel-render-controller.js`는 hosted panel snapshot 조립, `content/panel.js`는 iframe host + handle 위치/열림 상태 + bridge endpoint를 맡고, `content/hosted-panel-bridge.js`는 hosted iframe과의 postMessage contract를 맡는다. `content/panel-hosted-bridge-request.js`는 hosted iframe request의 `runtime/page/panel` domain 라우팅을 맡고, `content/panel-hosted-meeting-request.js`, `content/panel-hosted-prompt-request.js`, `content/panel-hosted-runtime-request.js`, `content/panel-hosted-page-request.js`, `content/panel-hosted-shell-request.js`는 그 아래에서 domain/action별 top-panel helper를 맡는다. 현재 `1.0.0+` extension bundle은 panel을 `content/panel-v2-composition-controller.js`로 직접 부팅하고, `backup/legacy-panel/panel-composition-controller.js`, `backup/legacy-panel/panel-action-controller.js`, `backup/legacy-panel/panel-meeting-controller.js`, `backup/legacy-panel/meeting-manager.js`는 활성 manifest에 싣지 않는다. `1.0.0+` v2 lane은 shell/runtime/route/page adapter만 extension에 남기고, tool rail/header chrome 같은 hosted shell 결정은 더 이상 extension에서 계산하지 않는다. v2 bootstrap은 실제로 hosted panel이 아직 보내는 shell callback(`select-tool`, `search`, `bookmark`, `release`, `escape`, `meeting-summary-sync`)만 host에 넘기고, prompt/meeting legacy action callback은 기본 surface에서 제외한다. `DB/Functions` 계약을 바꾸지 않는 순수 panel migration이라면 판단 기준은 현재 v2 bundle이 정상 동작하는지다. v2 composition은 prompt bridge proxy를 다시 한 번 만들지 않고 hosted-owned prompt controller를 shell/render/bootstrap wiring에 직접 넘긴다. legacy lane도 별도 prompt bridge proxy 없이 `backup/legacy-panel/panel-prompt-controller.js`를 shell/render/bootstrap에 직접 넘긴다. conversation도 v2에서는 bookmark item list와 empty/meta 문구를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/conversation-controller.js`가 page snapshot으로 직접 계산하며 extension은 count, activeId, refresh용 fingerprint만 남긴다. prompt도 v2에서는 prompt/store item list를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/prompt-library-controller.js`, `prompt-library-firestore-client.js`, `prompt-store-controller.js`, `prompt-review-controller.js`가 각 탭 상태와 prompt tab 전환/persistence, prompt/store/review action routing, import/reorder draft mutation을 직접 계산하며 extension은 `content/panel-v2-prompt-controller.js`를 통해 activeTab과 review handoff signal, composer review float만 유지한다. active `1.0.0+` manifest는 더 이상 `backup/legacy-panel/features/prompt-library/files.js`, `backup/legacy-panel/features/prompt-library/cloud-sync-manager.js`, `backup/legacy-panel/features/prompt-library/prompt-manager.js`, `backup/legacy-panel/features/prompt-store/prompt-realtime-manager.js`, `backup/legacy-panel/features/prompt-store/store-manager.js`, `backup/legacy-panel/prompt-hub-state.js`, `backup/legacy-panel/prompt-hub-panel.js`, `backup/legacy-panel/prompt-hub-controller.js`, `backup/legacy-panel/prompt-hub-runtime.js`, `backup/legacy-panel/panel-prompt-controller.js`를 활성 bundle에 싣지 않는다. 프롬프트 `내 요청` read path는 hosted panel 안에서 `auth.issue-prompt-panel`로 Firebase custom token을 발급받고, `integration_inova_accounts_v2.promptLibraryMeta` `onSnapshot` 뒤 `prompt_library_orders_v2`/`prompt_library_chunks_v2` direct read로 조립한다. 단순 목록 read를 위한 `loadInovaPromptLibraryV2` Functions fetch는 v2 hosted path에서 다시 쓰지 않는다. release도 v2에서는 latest/history/error 같은 상세 view-state를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/release-controller.js`가 직접 계산하며 extension은 handle/rail count용 update 신호만 남긴다. meeting도 v2에서는 회의 허브 item list와 action UI 상태를 extension snapshot에 싣지 않고, hosted `extension-v2/panel/meeting-hub-controller.js`가 `storage.get-state`, `auth.issue-meeting-panel`, hosted `meeting-firestore-client.js`, `meeting.open-*`, `meeting.*share*` runtime invoke로 목록/오류/degraded/pending/feedback/share 상태를 직접 계산한다. 목록 읽기는 hosted panel 안에서 Firebase custom token sign-in 뒤 Firestore `integration_inova_meetings` `onSnapshot`으로 이어지고, hosted meeting hub는 compact `meeting summary`를 top panel로 다시 올려 extension snapshot의 `count`와 `snapshotFingerprint`만 유지한다. 이 compact summary는 v2 composition 안의 `meetingSummary` 상태로 바로 normalize/fingerprint 처리하며, `backup/legacy-panel/meeting-manager.js` merge helper를 다시 거치지 않는다. v2 hosted panel은 `meeting-action` top-panel fallback 없이 open/share/revoke를 자체적으로 끝내며, extension은 runtime broker와 브라우저 탭 열기만 맡는다. v2 shell도 route/storage/prompt-tab/surface/bootstrap/visibility/focus 같은 sidecar/lifecycle 이벤트로는 더 이상 `backup/legacy-panel/meeting-manager.js` sync를 깨우지 않고, hosted `meeting-hub-controller`가 snapshot fingerprint, meeting tool 재진입, hosted 문서 visibility/focus, explicit action 결과를 기준으로 load/refresh를 직접 판단한다. meeting storage listener는 v2 bootstrap에서 등록하지 않고, tool state/controller ownership은 hosted `extension-v2/panel/*`로 점진 이동한다. 회의 기능은 브라우저 쪽에서 `meetingStateByMeetingId` 상태만 유지하고, `backup/legacy-panel/meeting-manager.js`는 legacy reference/source로만 남는다. 회의 허브 목록은 별도 `chrome.storage.local` 캐시 대신 hosted Firestore subscription과 현재 메모리 상태를 우선한다. 프롬프트/스토어 쪽은 legacy lane에서 `backup/legacy-panel/panel-prompt-controller.js`가 `backup/legacy-panel/prompt-hub-*` shell과 `backup/legacy-panel/features/prompt-library/cloud-sync-manager.js`, `backup/legacy-panel/features/prompt-store/prompt-realtime-manager.js` wiring을 감싸고 panel shell도 직접 상대한다. `1.0.0+` v2 lane에서는 hosted prompt panel이 `prompt-tab-select`, `prompt-action`, `prompt-draft-change`, `store-action`, `import-file`, `move-prompt` 같은 top-panel fallback 없이 자체 tab selection과 action flow를 끝낸다. `1.0.0+` v2 lane은 `v2.*` local storage key와 `integration_inova_accounts_v2` prompt-library meta 문서를 우선한다. panel/read-state 계약은 공통으로 `source`, `degraded`, `degradedReason`, `dataFreshness(fresh|stale|empty)`를 함께 들고, stale cache를 success-like fallback으로 숨기지 않는다. 패널 디버그는 회의 전용이 아니라 현재 브라우저 탭 세션 기준 전역 버퍼를 쓰고, 사용자 관측은 top 콘솔 trace를 기준으로 한다.
...
5. extension은 `content/panel-v2-prompt-controller.js`를 통해 `activeTab`과 review handoff signal, composer review float만 유지한다.
...
- backup legacy conversation/runtime reference가 필요할 때만 `npm run verify:legacy-backup` 안의 `node scripts/verify-panel-bookmark-controller.js`, `node scripts/verify-panel-runtime-controller.js`로 `backup/legacy-panel/panel-bookmark-controller.js`, `backup/legacy-panel/panel-runtime-controller.js` 계약을 다시 본다.
