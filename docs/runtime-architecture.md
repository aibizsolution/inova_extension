...
- 특징: `0.4.5`부터 panel 기본 UI는 content DOM이 아니라 hosted iframe 안에서 렌더링된다. bookmark/prompt/store/review/meeting/release view와 prompt tool 상호작용 helper는 여기서 렌더링하고, 확장은 iframe host/bridge와 page adapter를 유지한다. panel debug overlay는 iframe viewport에 갇히지 않도록 page DOM(`content/panel.js`)에서 별도로 그린다.
