...
- 특징: active shared root inventory는 `shared/constants.js`, `shared/firestore-collections.js`, `shared/firebase-config.js`, `shared/product-lane.js`, `shared/provider-identity-cache.js`, `shared/session.js`, `shared/storage.js`로 고정하고, 이 목록은 `contracts/extension-contract.json` + `scripts/verify-contracts.js`로 같이 잠근다. 이때 Firestore collection name catalog는 `shared/firestore-collections.js`가 소유하고, `shared/firebase-config.js`는 hosting/panel/meeting target과 browser-agnostic config만 유지하며, Functions endpoint family나 lane-specific endpoint override는 다시 들지 않는다.
