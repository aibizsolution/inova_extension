...
- 특징: Functions endpoint family와 lane-aware local/prod runtime resolution은 active shared root가 아니라 `background/functions-runtime-config.js`가 소유한다. `background/cloud-api-client.js`, `background/panel-session-capability.js`, `background/meeting-workspace-capability.js`는 이 background-only runtime config를 재사용하고, `shared/product-lane.js`나 `shared/firebase-config.js`가 endpoint family 이름을 다시 들지 않는다.
- 특징: 장기 운영 목표는 backend action 추가 때문에 extension ZIP을 재배포하지 않는 것이다. 현재 `background/panel-runtime-capability-router.js`와 `background/functions-runtime-config.js`는 bundled manifest baseline을 extension 내부에 두고, `functions-runtime-config.js`가 trusted Hosting `capability-manifest.json`을 fetch/cache한다. 이 fetch/cache는 검증 실패를 warning/degraded로 드러내고 bundled baseline으로 돌아가며, 다음 구조 개선은 remote manifest의 endpoint/target resolution을 실제 `functions.invoke-endpoint` URL 해석에 적용하는 단계다. 단, 새 browser permission, 새 host permission, 새 page DOM adapter, 새 web-accessible asset처럼 extension 권한 자체가 바뀌는 경우는 계속 extension 재배포 대상이다.
