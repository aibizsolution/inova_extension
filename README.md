...
- `우측 슬라이드 패널`
  ...
  - 프롬프트 `내 요청` 카드의 날짜와 `관리` 버튼 메타 영역은 좁은 폭에서도 한 줄을 유지해, 날짜/버튼이 어색하게 두 줄로 갈라지지 않게 맞춥니다.
- `회의록 패널`
  - `회의` 도구는 Firestore 회의 문서를 실시간으로 구독해 최신 회의록 목록과 `새 회의하기` CTA를 제공합니다.
  - 회의 패널 상단의 목록 요약은 별도 큰 카드 대신 더 낮은 밀도의 요약 행으로 보여, 실제 회의록 카드가 먼저 눈에 들어오게 유지합니다.
  ...
  - `공유 해제` 버튼은 실제 활성 공유가 있을 때만 표시하고, 공유 전 상태에서는 아예 숨겨 패널 액션을 더 단순하게 유지합니다.
  - 회의 패널 액션 성공은 버튼 상태 변화로 바로 확인되도록 하고, 별도 파란 성공 배너는 띄우지 않습니다. 대신 실패 안내만 카드 상단에 남겨 사용자가 실제 문제만 바로 보게 유지합니다.
  - 회의 패널 상단에는 액션 성공/진행용 임시 안내를 따로 띄우지 않고, 버튼 상태 변화와 실패 배너만으로 상태를 전달합니다.
...
- 완료된 기록 탭 줄 오른쪽에는 공통 `회의록 업데이트` 액션을 두고, 기록 메모 또는 저장된 추가 맥락이 마지막 회의록 반영 입력과 다를 때만 활성화합니다.
...
- meeting 작업실의 쓰기 액션은 HTTP로 요청만 접수하고, 실제 완료·실패와 최신 내용 반영은 Firestore snapshot 구독 상태로 판단합니다.
- `회의록 업데이트`는 비동기 재생성 요청을 접수한 뒤, job/artifact snapshot에 반영되는 진행 상태와 결과에 맞춰 완료 여부와 오류를 표시합니다.
...
- `기록 메모`와 `추가 맥락` 입력창은 사용자가 입력 중인 띄어쓰기와 줄바꿈을 그대로 유지하고, 저장/전송 시점에만 정규화합니다.
- `추가 맥락` 탭에서는 항목 추가·수정·삭제를 바로 저장하고, 저장된 목록이 마지막 회의록 반영 입력과 달라졌을 때만 `회의록 업데이트`를 활성화합니다.
- 메모/추가 맥락 저장이 최신 함수 계약과 맞지 않으면 프런트에서 성공처럼 숨기지 않고, `npm run deploy:functions`가 필요하다는 오류를 그대로 보여줍니다.
  - 회의는 현재 대화 세션과 분리된 `meetingId` 기준으로 관리하고, 같은 회의의 처리 이력만 페이지 안에 남깁니다.
- 좌측 `기록 선택` 카드는 AI 내부 판단보다 제목, 상태, 마지막 정리 결과를 중심으로 보여줘서 어떤 기록을 다시 열어야 하는지 빠르게 구분할 수 있게 유지합니다.
- 상단 `회의 작업` 영역에서는 회의 이름과 공용 메모를 저장할 수 있고, 우측 `기록 상세` 패널에서는 완료 기록 기준 `회의 정리 / 메모 / 원문 / 추가 맥락` 탭을 전환합니다. 상단 카드에서는 개별 기록 이름 수정과 삭제를 처리하며, 삭제를 실행하면 연결된 job/artifact와 남아 있는 임시 source object까지 함께 정리합니다.
...
- 원격 처리 중 상태 갱신은 작업실이 `확장 bridge authorize -> authorizeInovaMeetingWorkspaceAccess -> Firebase Auth`를 거친 뒤 Firestore `meeting/job/artifact` 문서를 직접 구독해 반영합니다. Functions는 업로드/삭제/재정리 같은 명령만 맡고, 탭 복귀 시에는 끊긴 listener만 다시 연결합니다.
- `새 회의하기`처럼 아직 회의 문서가 비어 있는 첫 진입도, authorize 단계에서 확보한 `meetingId`에 한해 Firestore listener를 먼저 붙일 수 있게 유지합니다.
- hosted 작업실은 boot 시 로컬 세션/보관 큐만 준비되면 빈 작업실 shell을 먼저 렌더하고, 첫 Firestore snapshot은 짧게만 기다린 뒤 늦으면 뒤에서 이어 받아 체감 로딩을 줄입니다.
- 기본 URL은 `meeting/index.html?meetingId=...&jobId=...` clean URL만 유지하고, 예전 `launch/#ws` 링크는 더 이상 권한을 주지 않고 blocked 안내만 보여 줍니다.
...
- `authorizeInovaMeetingWorkspaceAccess` 서버 경로는 bearer access token만으로 현재 사용자를 역산하지 않고, background가 함께 보낸 `providerIdentity.providerUserKey` 힌트를 기준으로 `verifyInovaIdentity()`를 통과시킵니다. 그래서 secure authorize body에서 이 필드를 빠뜨리면 direct clean URL이 `identity-required`로 막힙니다.
- `대화 안에서 찾기`
...
- `프롬프트 스토어`
  ...
  - `내 등록`은 별도 owner read 대신 이미 로드된 전체 목록에서 현재 사용자 항목만 로컬 필터해 보여줍니다.
  - 카테고리와 정렬 UI는 불필요한 sync와 동일 markup 재주입을 줄여 깜빡임을 완화합니다.
...
- `릴리스 안내`
  ...
  - 현재 배포 안내 카드는 패딩과 세로 간격을 줄여, 요약과 다운로드 버튼을 같은 화면에서 더 빽빽하지 않게 읽도록 유지합니다.
...
- `background/`
  - `service-worker.js`: 외부 네트워크 호출과 클라우드 백업, prompt/store panel auth, 회의 허브 조회, hosted 작업실 clean URL opener, 확장 bridge 기반 workspace authorize/share-link 라우팅
...
- `background/service-worker.js`는 i-Nova access token과 Firebase Functions를 연결해 원격 백업 호출과 prompt/store panel auth 발급을 처리하고, 회의 기능에서는 hosted 작업실 clean URL 생성, secure authorize 요청 중계, 공유 링크 생성/해제를 맡습니다.
- `functions/features/meeting/meeting-workspace-auth-service.js`는 hosted 작업실 `owner-secure`/`share-readonly` 접근 판정, 공유 링크 create/revoke, Firestore 읽기용 Firebase custom token 발급을 맡깁니다. `meeting-launch-service.js`는 레거시 launch/session 호환만 남깁니다.
- hosted meeting 보안 모델처럼 Firebase custom token claim과 Firestore read rules가 함께 바뀌는 작업은 `hosting/functions`만 배포하면 불완전합니다. 이 저장소의 전체 배포는 `firestore.rules`까지 포함해 `npm run deploy:all` 또는 `npm run deploy:firestore`를 함께 써야 실제 권한 경계가 맞춰집니다.
...
9. 녹음이 `90분`에 도달하면 현재 기록은 자동으로 전사 큐에 들어가고, 회의 화면은 다음 개별 기록 녹음을 이어갑니다.
10. 저장된 결과를 선택하면 우측 `기록 상세` 패널 상단에서 이름을 수정하거나 삭제하고, 검토 탭에서 자동 정리와 발화 구간 기준 전사를 함께 확인할 수 있습니다.
...
16. 필요하면 요청 묶음을 JSON으로 내보내거나, 다른 사용자의 요청 묶음을 가져옵니다.
...
```text
http://127.0.0.1:5000/meeting/index.html?...&debug=1&debugAuthBypass=owner&recordLimitSeconds=30
```
