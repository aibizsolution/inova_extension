...
- `hosting/meeting/`
  ...
  - `index.js`: hosted 회의 작업실 부팅, launch token 교환, 세션 복원, 녹음/업로드 큐/상세 액션 orchestration과 queue diagnostics consume/error wrapper, queue-backed action error notice 처리, retry reset/upload cleanup 경계와 superseded local request 정리, retry/restart transition helper, remote sync 이후 pending queue 상태를 `upsertPendingUpload()` 경계로 다시 합치는 reconciliation, 이해하지 못한 remote status를 silent fallback 대신 warning/log로 surface하는 전이 규칙
