...
- 패널 디버그는 회의 탭 전용이 아니라 현재 브라우저 탭 세션 기준 전역 버퍼로 유지되고, `대화/회의/프롬프트/릴리스` 흐름 로그를 함께 모읍니다.
  ...
  - hosted 작업실 디버그가 켜져 있으면 브라우저 콘솔에서 `__INOVA_HOSTED_MEETING_DEBUG__.setFault(name, count)`로 queue storage fault를 주입해 수동 검증할 수 있고, 가능한 이름은 `__INOVA_HOSTED_MEETING__.storage.DEBUG_FAULTS`에서 바로 확인할 수 있습니다. `clearFault(name)` 또는 `clearFault()`로 개별/전체 fault를 비울 수 있습니다.
