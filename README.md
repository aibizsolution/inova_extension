...
queue degraded 수동 검증만 빠르게 하려면 localhost 작업실을 `http://127.0.0.1:5000/meeting/index.html?debug=1&debugQueueSandbox=1`로 열어 로컬 queue sandbox를 먼저 띄울 수 있습니다. 이 모드에서는 panel/session 없이도 import/hold/delete/reload 중심의 queue load/persist/cleanup 검증을 로컬에서 반복할 수 있고, 원격 refresh/retry는 건너뜁니다.
