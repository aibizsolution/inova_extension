...
- 상용 Functions의 기본 AI provider는 OpenRouter first입니다. 회의 전사는 OpenRouter 실패 시 같은 JSON secret 안의 Gemini 설정을 Files API fallback으로 먼저 쓰고, OpenAI는 필요한 경우 마지막 fallback으로만 둡니다. Secret Manager에는 provider별 개별 키를 늘리지 않고 저장소 전용 JSON secret `INOVA_EXTENSION_AI_PROVIDER_CONFIG` 하나만 둡니다. 공용 `OPENAI_API_KEY` secret이나 `.env`에 i-Nova/OpenRouter/Gemini 키를 넣어 배포하지 않고, 로컬 에뮬레이터는 ignored `functions/.secret.local`을 사용합니다.
