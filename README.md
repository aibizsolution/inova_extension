...
- 상용 Functions의 기본 AI provider는 OpenRouter first이지만, 회의 전사는 비용 정책상 Gemini Files API를 primary로 쓰고 OpenRouter/OpenAI를 fallback으로 둡니다. Secret Manager에는 provider별 개별 키를 늘리지 않고 저장소 전용 JSON secret `INOVA_EXTENSION_AI_PROVIDER_CONFIG` 하나만 둡니다. 공용 `OPENAI_API_KEY` secret이나 `.env`에 i-Nova/OpenRouter/Gemini 키를 넣어 배포하지 않고, 로컬 에뮬레이터는 ignored `functions/.secret.local`을 사용합니다.
