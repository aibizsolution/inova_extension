...
- 패널 디버그는 회의 탭 전용이 아니라 현재 브라우저 탭 세션 기준 전역 버퍼로 유지되고, `대화/회의/프롬프트/릴리스` 흐름 로그를 함께 모읍니다.
  ...
  - hosted 작업실 세션/큐 degraded 경고는 `저장값 없음`, `session/local storage 접근 실패`, `파싱 실패`, `저장 실패`, `삭제 실패`, `stale refresh`를 같은 결과로 뭉개지 않고, 우선순위가 있는 scoped warning UI와 디버그 로그에서 구분해 보여줍니다.
