...
- panel shell 변경은 먼저 `content/main.js`, `content/panel-v2-composition-controller.js`, `content/panel-v2-shell-bridge.js`, `content/panel.js`, `content/panel-console-trace.js`, `content/hosted-panel-bridge.js`, `content/route-state-controller.js`, `content/route-watch-controller.js`, `content/route-sync.js`, `content/panel-v2-prompt-controller.js`의 책임 경계를 확인하고 시작한다.
...
- panel console trace 정책과 snapshot trace payload helper는 `content/panel.js` 안에 다시 섞지 말고 `content/panel-console-trace.js`에서 유지한다. `panel.js`는 trace helper를 주입받아 iframe host + bridge role만 유지한다.
- 현재 `content/hosted-panel-bridge.js`의 active request helper는 v2 hosted panel이 실제로 쓰는 `summary sync`, `conversation bookmark`, shell/runtime/page contract만 유지한다. legacy `meeting-action`, `release-action`, prompt action request path는 active `1.0.0` bundle의 보존 대상이 아니다.
