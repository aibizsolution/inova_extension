...
- panel 회의 허브에서는 버튼뿐 아니라 제목, 목록, 빈 상태, degraded 안내도 모두 그룹 단위로 읽히게 맞춘다. 새 그룹 생성 액션은 세션 단위 시작처럼 보이지 않게 `새 회의하기`보다 `새 회의 룸 생성` 같은 용어를 우선하고, 리스트 표면도 `회의록`보다 `회의 룸`을 우선한다.
