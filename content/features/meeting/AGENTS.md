...
- v2 meeting snapshot shaping은 `panelMeetingController`의 action UI state를 거치지 않고 raw `meetingHub` summary만으로 `count`와 fingerprint를 만든다. action fallback controller는 mixed-version/runtime fallback 경로에만 남긴다.
