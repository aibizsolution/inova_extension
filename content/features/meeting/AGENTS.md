...
- v2 meeting snapshot shaping은 `panelMeetingController`의 action UI state나 `content/meeting-manager.js` merge helper를 거치지 않고, hosted가 올린 raw `meetingHub` summary만으로 `count`와 fingerprint를 만든다. action fallback controller는 mixed-version/runtime fallback 경로에만 남기고, v2 부팅 시에는 eager 생성하지 않는다.
