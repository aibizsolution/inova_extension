...
- 같은 이유로 v2 panel shell lifecycle(`open/close`, `visibility`, `surface`, `route`, `storage`)는 더 이상 `content/meeting-manager.js` 인스턴스를 깨워 회의 동기화를 직접 예약하지 않는다. hosted meeting hub가 snapshot fingerprint와 explicit action을 기준으로 자체 load/refresh를 판단한다.
