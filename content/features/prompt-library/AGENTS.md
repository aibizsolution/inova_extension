...
- `backup/legacy-panel/shared/prompt-library.js` - inactive helper/reference, active manifest preload 아님
...
- active `1.0.0+` manifest는 더 이상 prompt import/export helper를 content preload로 싣지 않는다. `backup/legacy-panel/shared/prompt-library.js`는 dormant helper reference용으로만 남기고, active prompt shell/runtime 판단 기준은 hosted controller와 `shared/provider-identity-cache.js`, `shared/storage.js`, `content/provider-identity-sensor.js` 쪽에 둔다.
- active `shared/provider-identity-cache.js`와 `shared/storage.js`는 v2 lane에서 generic provider-identity/local-state merge만 맡는다. legacy cloud sync base/helper와 dormant prompt CRUD/sync operation helper, local cache schema는 `backup/legacy-panel/shared/cloud-sync.js`, `backup/legacy-panel/shared/prompt-cloud-sync.js`, `backup/legacy-panel/shared/prompt-storage.js`, `backup/legacy-panel/shared/prompt-library.js` reference로만 두고 active manifest preload에 다시 싣지 않는다.
