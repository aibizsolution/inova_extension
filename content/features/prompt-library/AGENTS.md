...
## 구현 경계
- 내 요청 보관함은 인터넷 연결 없이는 쓸 수 없는 제품 전제를 따른다. prompt library의 정본은 Firestore/Functions가 들고, active `1.0.0+` lane은 `chrome.storage.local.promptLibrary`를 source-of-truth로 취급하지 않는다. retired local cache reference는 보존하지 않는다.
- `0.4.4` retirement 이후 legacy prompt-library reference source는 보존하지 않는다. 과거 동작 확인이 필요하면 git history를 사용한다.
...
- active `1.0.0+` manifest는 더 이상 prompt import/export helper를 content preload로 싣지 않는다. active prompt shell/runtime 판단 기준은 hosted controller와 `shared/provider-identity-cache.js`, `shared/storage.js`, `content/provider-identity-sensor.js` 쪽에 둔다.
- active `shared/provider-identity-cache.js`와 `shared/storage.js`는 v2 lane에서 generic provider-identity/local-state merge만 맡는다. legacy cloud sync base/helper와 dormant prompt CRUD/sync operation helper, local cache schema는 active manifest preload나 backup reference로 다시 추가하지 않는다.
...
- active `1.0.0+` extension state factory와 route hydration은 hosted-owned prompt library/store/editor mirror를 다시 들지 않는다. `chrome.storage.local.promptLibrary` 캐시를 active extension `createState()`나 `route-state-controller`가 그 목록/편집/store bucket에 다시 올리거나 active shared storage contract에 계속 싣고 있으면 안 된다.
...
## lane 경계
- retired 0.4.x prompt-library reference source는 repo에 보존하지 않는다.
