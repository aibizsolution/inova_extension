...
- active `1.0.0+` manifest는 더 이상 `shared/prompt-library.js`를 content preload로 싣지 않는다. 이 파일은 import/export 정규화와 dormant storage/helper reference용으로만 남기고, active prompt shell/runtime 판단 기준은 hosted controller와 `shared/cloud-sync.js`, `shared/provider-identity.js` 쪽에 둔다.
