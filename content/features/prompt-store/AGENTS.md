...
- `0.4.4` retirement 이후 legacy store reference source는 보존하지 않는다. 과거 동작 확인이 필요하면 git history를 사용한다.
...
- popup의 `settings.meetingWorkspaceTarget=local`을 고르면 prompt-store도 local full-stack rehearsal로 같이 전환돼야 한다. local target은 `http://127.0.0.1:5000/extension/prompt-panel-bridge.html`을 향하고, i-Nova page CSP의 local frame 허용은 background `declarativeNetRequest` rule이 맡는다. page DOM에 extension `content/frame-proxy.html?target=...` wrapper를 꽂는 방식은 Playwright Bridge attach를 깨므로 emergency-only fallback으로만 둔다. prompt panel auth/read/write는 local Functions/Auth/Firestore emulator 경로를 써야 한다.
